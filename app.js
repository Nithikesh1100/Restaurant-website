...
searchInput.addEventListener("input", e => {
  ...
  const menuHTML = generateMenu(filteredMenu);
...
function generateMenu(menu) {
  return menu.map(item => `
      <div class="menu-item col-sm-12 col-md-6 col-lg-4  my-4">
...
function displayMenuByCategory(category) {
  ...
  const menuContainer = document.getElementById('menu-container');
  const menuHTML = generateMenu(filtered);
  menuContainer.innerHTML = menuHTML;
}
...
function starters() {
  displayMenuByCategory('starters');
}

function beverages() {
  displayMenuByCategory('beverages');
}
